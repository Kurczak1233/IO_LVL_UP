<template>
    ...
    <b-container v-if="formalConsent === false" class="col-10 mt-5 col-md-8 col-xl-8 mr-auto ml-auto background-bluish" fluid>
     ...
     <b-container v-if="formalConsent === true" class="col-12 col-sm-10 mt-5 col-md-8 col-xl-8 mr-auto ml-auto background-bluish p-5" fluid>
        <b><h3><i>John Prince, famous dancer and choreographer, gives advice on how to succeed in a career in the arts.</i></h3></b>
...
<b-row>
    <b-col>
        <b>Sentences</b>
<p><b>A</b>. It's fine, but I try not to give out too much advice as it gets irritating!</p>

<p><b>B</b>. And if nothing you like comes out of it, then come back and be an actor or dancer.</p>

<p><b>C</b>. Without a strict daily timetable like this you find yourself wasting too much time.</p>

<p><b>D</b>. After that it's back to England to start a new term of dance classes.</p>

<p><b>E</b>. When it comes to coping with stress, I find that exercise helps me to cope with my problems, so I stay in good shape mentally as well.</p>

<p><b>F</b>. Like any profession where you're always travelling, you tend to acquire something new almost every day.</p>

<p><b>G</b>. Being fully equipped with all this stuff beforehand makes it easier when you go for auditions.</p>
    </b-col>
</b-row>
</form>
            <b-row>  
                <b-col class="mt-3 mb-3 text-center"><router-link :to="{name: 'read', params: {solvedB1Read5: true}}" ><b-button class="btn btn-warning" type="button" v-on:click="AddSolvedToUserDb">Solved!</b-button></router-link></b-col>
            </b-row>   
     </b-container>
    </body>
</template>
...
export default {
    ...
    {
        checkForm: function() {
                let correctAnswears = ["G", "E", "D", "A", "F", "B"];
  ...
  {
      this.solvedB1Read5 = true;
      var db = firebase.firestore();
      db.collection(this.email).doc(this.email).set({solvedB1Read5: this.solvedB1Read5} ,{merge:true})
  }
    }
}
</script>
